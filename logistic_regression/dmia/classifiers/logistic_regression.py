...
class LogisticRegression:
    ...
    def train(self, X, y, learning_rate=1e-3, reg=1e-5, num_iters=100, batch_size=200, verbose=False):
        ...
        for it in range(num_iters):
            ...
            # В случайном порядке выберем batch_size элементов из train-выборки
    ...
    def predict_proba(self, X, append_bias=False):
        ...
        y_predictions = self.calc_predictions(X, self.w)
    ...
    def loss(self, X_batch, y_batch, reg):
        ...
        y_predictions = self.calc_predictions(X_batch, self.w)
        loss = -np.sum(y_batch * np.log(y_predictions) + (1.0 - y_batch) * np.log(1.0 - y_predictions))

        dw = (self.calc_predictions(X_batch, self.w) - y_batch).T * X_batch
